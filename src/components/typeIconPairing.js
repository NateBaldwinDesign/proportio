...
const TypeIconPairing = (props) => {
    ...
    const scale = (textIconGapScaleMethod === 'typeScale') ? typeScale: ((textIconGapScaleMethod === 'spacingScale') ? spacingScale : 'none');
    const method = (textIconGapScaleMethod === 'typeScale') ? typeScaleMethod: ((textIconGapScaleMethod === 'spacingScale') ? spacingScaleMethod : 'none');
    
    const smallSizes = smallSizeArray.map((e, i) => {
        const increment = ((1+ i) * -1) + gap;
        const iconIncrement = i - textIconIconSizeIndex;
        const gapSize =  Math.round(calculateScale(baseSize, scale, increment, method));

        return <div className="textIconItem">
            ...
            <div className="typeIconPair" key={`typeIcon-neg${i}`}>
                ...
                />
                <span style={{display: 'block', margin: 0, padding: 0, width: `${gapSize}px`}}></span>
                <TypeElement
